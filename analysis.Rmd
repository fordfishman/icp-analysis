...
```{r}
setwd("~/Documents/Geology_program") # Set working directory
file <- "Field Foams 28,38,39,75,176,186,191,195,212.xlsx"
limits <- "/Users/fordfishman/Documents/Geology_program/MDL-LOQ limits.xlsx"
weights <- "Masses of dried field foams (mod 1-06.27.19).xlsx"
rsquare_cutoff <- 0.94
sample_start <- "A"
sample_end <- "C"
output <- "IPC_analysis.xlsx"
load("IPC-analysis-app/data/IPC_data.Rdata") # This data includes background foam concentrations and weights
```
...
The drift correction is made by multiplying the concentrations by the inverse of the drift multipler found above that is unique for each sample. The drift corrected data is stored in the dataframe `drift_corrections`.
...
Now that drift has been accounted for, the next, much simpler step is to substract out the blank concentrations as determined from the CCB samples. The mean of each analyte in the CCB samples is subtracted from the drift-corrected values, which is stored in the dataframe `drift_corrections`.
...
`data_range` is defined as the the range of columns in `corrected_data` that actually contains analyte data.
...
`QA_QC` is then converted into a dataframe to easily convert it into an Excel sheet later on. But before the dataframe is finalized, the three tests with multiple samples have to removed from the rest of test results.
...
For any given sample, each analyte concentation mean has to compared to background foam concentration mean in an independent samples 1-tailed t-test, where we we are testing the hypothesis that the mean concentration in the sample is greater than the mean concentration in the background foam. The custom function `sig_test()` lets you run all of the necessary t-tests for each analyte and returns each p-value in a vector. If a sample only has one replicate, a t-test will not work, and a p-value of 1 will be returned. 
...
Here's where those functions get put to use: in a monster for-loop. Subsets are made only containing a single sample from the `sample_norm` (weight-normalized) and `sample-df` (non-normalized concentrations) dataframes. The non-normalized concentrations (`conc_mean`) are used to determine the labeling discussed above via boolean vectors, besides the signficance vector.
